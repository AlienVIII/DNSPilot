...
- Automated tests also check `x:Uid` localization hooks, `en-US`/`vi-VN` resource keys, dynamic Vietnamese shell text, package capability template, PNG asset dimensions, bundled CLI copy rule, privacy draft, Store listing copy, and certification notes.
