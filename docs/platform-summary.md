...
| Mobile | Expo/React Native bridge shell with benchmark, diagnostics, storage forms, guided settings, System Access recovery, native settings actions, localization, device setup checks, Expo SDK 57, and Android debug build evidence. | Meets test-shell and mobile-policy exploration requirements. | Native Rust adapter/backend decision, iOS/Android real-device QA, iOS 26.5 Simulator runtime, store account/signing flows. |
